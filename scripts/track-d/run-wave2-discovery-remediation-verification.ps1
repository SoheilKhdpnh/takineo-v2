...
Write-Host ''
Write-Host 'Engineering Control Room closure policy:' -ForegroundColor Cyan
Write-Host ''
Write-Host 'Required at BOTH 1k and 10k:'
Write-Host '  candidate lookup is projection-only'
Write-Host '  no eligibility User joins during candidate hunting'
Write-Host '  no eligibility IntroVideo joins during candidate hunting'
Write-Host '  teacherProfileRowsFetched = O(page size)'
Write-Host '  availability query count remains constant'
...
Write-Host ''
Write-Host 'Required at representative larger scale (10k):'
Write-Host '  projection indexed/keyset candidate traversal remains page-bounded'
Write-Host '  profile and availability work do not grow with total teacher population'
Write-Host '  no recurrence of population-dependent eligibility hunting'
Write-Host ''
Write-Host 'Small-table PostgreSQL planner allowance:'
Write-Host '  Sequential scans at the 1k fixture are non-blocking when PostgreSQL chooses'
Write-Host '  them for tiny relations, provided the query architecture remains projection-only,'
Write-Host '  contains LIMIT pageSize+1, performs no eligibility joins or OFFSET filtering,'
Write-Host '  and the representative larger-scale plan proves bounded indexed traversal.'
Write-Host ''
Write-Host 'Physical rows examined at 1k are diagnostic, not a standalone closure condition.'
