...
const mocks = vi.hoisted(() => ({
  ...
  reconcileMuxPlayback: vi.fn(),
  queueMuxPlaybackIntent: vi.fn(),
}));
...
vi.mock("@/lib/services/mux-playback-reconciliation.service", () => ({
  queueMuxPlaybackIntent: mocks.queueMuxPlaybackIntent,
  reconcileMuxPlayback: mocks.reconcileMuxPlayback,
}));
...
describe("administrative operator service boundaries", () => {
  beforeEach(() => {
  ...
  it("requires MANAGE_ADMIN_ACCESS for permission changes and audits the reason", async () => {
    const tx = {
      user: {
  ...
  it("refuses to revoke the last active SUPER_ADMIN", async () => {
    ...
    expect(tx.adminAccess.update).not.toHaveBeenCalled();
    expect(tx.adminAuditEvent.create).not.toHaveBeenCalled();
  });

  it("requires MODERATE_ACCOUNT for account state changes", async () => {
    const tx = {
      user: {
        findUnique: vi.fn().mockResolvedValue({
          accountStatus: "ACTIVE",
